        ...
        <div class="vc_col-md-12 aux-vc-templates-library">
            ...
            if ( ! empty( $category['templates'] ) ) {
                foreach ( $category['templates'] as $template ) {
                    ?>
                    <div class="vc_ui-panel-template-item vc_ui-visible" data-template-id="<?php echo esc_attr( $template['id'] ); ?>">
                        <span class="vc_ui-panel-template-item-content">
                            <img src="<?php echo esc_url( $template['thumbnailUrl'] ); ?>" alt="">
                            <span class="vc_ui-panel-template-item-overlay">
                                <a href="<?php echo esc_url( $template['previewUrl'] ); ?>" class="vc_ui-panel-template-item-overlay-button vc_ui-panel-template-preview-button" data-preview-url="<?php echo esc_attr( $template['previewUrl'] ); ?>" data-title="About Page With Features" data-template-id="<?php echo esc_attr( $template['id'] ); ?>" data-template-version="<?php echo esc_attr( $template['version'] ); ?>"><i class="vc-composer-icon vc-c-icon-search"></i></a>
                                <?php
                                if ( empty( $local_template_keys ) || ! in_array( $template['id'] , $local_template_keys )
                                    ...
                                    ?>
                                <a href="javascript:;" target="_blank" class="vc_ui-panel-template-item-overlay-button vc_ui-panel-template-download-button auxin-vc-template-download" data-n="<?php echo wp_create_nonce( 'auxin_vc_template_download' ); ?>" data-template-id="<?php echo esc_attr( $template['id'] ); ?>">
